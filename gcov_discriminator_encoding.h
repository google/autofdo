...
// Strip only multiplicity bits, keeping base and copy_id
// Used for MAX aggregation where we want to aggregate per (line, base, copy_id)
// but not create separate entries for different multiplicities
inline uint32_t StripMultiplicity(uint32_t discriminator) {
  uint32_t base = GetBaseDiscriminator(discriminator);
  uint32_t copy_id = GetCopyID(discriminator);
