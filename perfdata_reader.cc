...
// a) match_mmap_name == "" && binary has build_id:
//    the perf.data mmaps are selected using binary's build_id, if there is no
//    match, select fails.
// b) match_mmap_name == "" && binary does not has build_id:
//    select fails.
// c) match_mmap_name != ""
...
absl::StatusOr<BinaryMMaps> SelectMMaps(
  ...
  if (match_mmap_name.empty()) {
    ...
    mmap_selector = std::make_unique<MMapSelector>(
        absl::flat_hash_set<std::string>({std::string(match_mmap_name)}));
  }
  ...
  for (const auto &pe : perf_parser.parsed_events()) {
    ...
    // For kernel mmap event, pid is static_cast<uint32_t>(-1).
  ...
  if (binary_mmaps.empty()) {
    return absl::FailedPreconditionError(absl::StrCat(
        "Failed to find any mmap entries matching: '", match_mmap_name, "'."));
  }
...
uint64_t PerfDataReader::RuntimeAddressToBinaryAddress(uint32_t pid,
  ...
  if (i == binary_mmaps_.end()) return LbrAggregation::kInvalidAddress;
  const MMapEntry *mmap = nullptr;
  for (const MMapEntry &p : i->second)
    if (p.load_addr <= addr && addr < p.load_addr + p.load_size) mmap = &p;
  if (!mmap) return LbrAggregation::kInvalidAddress;

  uint64_t file_offset;
  bool is_kernel = (pid == static_cast<uint32_t>(-1));
  ...
  return LbrAggregation::kInvalidAddress;
}
...
void PerfDataReader::AggregateLBR(LbrAggregation *result) const {
  const bool is_kernel_mode =
      (binary_mmaps_.find(static_cast<uint32_t>(-1)) != binary_mmaps_.end());
  ...
  ReadWithSampleCallBack([&](const quipper::PerfDataProto::SampleEvent &event) {
    ...
    uint64_t last_from = LbrAggregation::kInvalidAddress;
    uint64_t last_to = LbrAggregation::kInvalidAddress;
    for (int p = brstack.size() - 1; p >= 0; --p) {
      ...
      if (last_to != LbrAggregation::kInvalidAddress && last_to <= from)
...
absl::StatusOr<PerfDataReader> BuildPerfDataReader(
  ...
  ASSIGN_OR_RETURN(BinaryMMaps binary_mmaps,
                   SelectMMaps(perf_data, match_mmap_name, *binary_content));
