...
void AddressRangeList::ReadRangeList(uint64 offset, uint64 base,
    AddressRangeList::RangeList* ranges, uint8 dwarf_version, uint64 addr_base) {
...
void AddressRangeList::ReadDwarfRangeList(uint64 offset, uint64 base,
  ...
  const char* pos = ranges_buffer_ + offset;
  do {
    CHECK((pos + 2*width) <= ranges_buffer_ + ranges_buffer_length_);
...
void AddressRangeList::ReadDwarfRngListsDirectly(uint64 offset, uint64 base,
  ...
  ReadDwarfRngLists(base, ranges, rnglist_buffer_ + offset, addr_base, i->second.address_size);
}
...
void AddressRangeList::ReadDwarfRngListwithOffsetArray(uint64 offset, uint64 base,
  ...
  ReadDwarfRngLists(base, ranges, rnglist_buffer_ + rnglistsdata.rnglist_base_ + offset, addr_base, rnglistsdata.address_size);
}
    ...
    do {
        ...
        switch (entry_kind) {
          ...
          case DW_RLE_base_address: {
            CHECK(pos + address_size <= rnglist_buffer_ + rnglist_buffer_length_);
...
void AddressRangeList::ReadDwarfRngListsHeader() {
    ...
    const char* ptr = rnglist_buffer_;
    ...
    do {
      ...
      CHECK(ptr + 12 < rnglist_buffer_ + rnglist_buffer_length_);
      // unit_length (initial length)
      size_t unit_length_size;
      rnglistsdata.unit_length = reader_->ReadInitialLength(ptr, &unit_length_size);

      CHECK(unit_length_size + rnglistsdata.unit_length <= rnglist_buffer_length_);
      ptr += unit_length_size;

      CHECK(ptr + 2 < rnglist_buffer_ + rnglist_buffer_length_);
      rnglistsdata.version = reader_->ReadTwoBytes(ptr);
      CHECK(rnglistsdata.version == 5);
      ptr += 2;

      CHECK(ptr + 1 < rnglist_buffer_ + rnglist_buffer_length_);
      ...
      CHECK(ptr + 4 < rnglist_buffer_ + rnglist_buffer_length_);
      rnglistsdata.offset_entry_count = reader_->ReadFourBytes(ptr);
      ptr += 4;
      
      rnglistsdata.rnglist_base_ = ptr - rnglist_buffer_;
    ...
    } while(ptr < rnglist_buffer_ + rnglist_buffer_length_);
...
{
    CHECK(*offsetarrayptr + reader_->OffsetSize() < rnglist_buffer_ + rnglist_buffer_length_);
