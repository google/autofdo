...
#define GCOV_TAG_MODULE_GROUPING (0xae000000)
#define GCOV_TAG_AFDO_WORKING_SET (0xaf000000)
#define GCOV_BLOCK_SIZE (1 << 10)
#define GCOV_DATA_MAGIC (0x67636461) /* "gcda" */
#define GCOV_ELF_SECTION_NAME ".gnu.switches.text"
...
static inline int gcov_open(const char *name, int mode) {
  CHECK(!gcov_var.file);
  gcov_var.start = 0;
...
static inline int gcov_close(void) {
  if (gcov_var.file) {
...
static inline void gcov_write_unsigned(uint32 value) {
  uint32 *buffer = gcov_write_words(1);
  buffer[0] = value;
}

static inline void gcov_write_counter(uint64 value) {
...
static inline void gcov_write_string(const char *string) {
  unsigned length = 0;
  unsigned alloc = 0;
  uint32 *buffer;
...
static inline uint32 gcov_read_unsigned(void) {
  uint32 value;
...
static inline uint64 gcov_read_counter(void) {
  uint64 value;
...
static inline const char * gcov_read_string(void) {
