...
ABSL_FLAG(bool, use_two_pass_aggregation, false,
          "Enable two-pass aggregation for copy_id: Pass 1 does MAX per "
          "(line, base, copy_id), Pass 2 does SUM after stripping copy_id. "
          "Should be enabled only for create_gcov with gcov_version=3.");
...
bool SymbolMap::AddIndirectCallTarget(absl::string_view symbol_name,
  ...
  // Use same offset calculation as AddSourceCount for consistency
  uint64_t offset;
  if (use_two_pass) {
